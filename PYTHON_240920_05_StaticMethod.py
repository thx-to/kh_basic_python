# 인스턴스 메소드 : 객체로 만들어질 때 함께 만들어지는 메소드, 일반적인 메소드(기본 메소드)
# 데코레이터(@) 없이 메소드 선언하면 인스턴트 메소드

# 정적 메소드 : 클래스와 관련 있음, 객체와 무관한 독립적인 동작 수행
# @staticmethod 데코레이터를 사용해 클래스에 메소드 선언하면 해당 메소드는 정적 메소드가 되며,
# 정적 메소드는 인스턴스 메소드나 클래스 메소드와 달리 첫번째 매개 변수가 할당되지 않음
# 정적 메소드 내에서는 인스턴스/클래스 속성에 접근하거나 인스턴스/클래스 메소드 호출하는 것이 불가능
# 유틸리티 메소드 구현시 많이 사용

# 클래스 메소드 : 클래스와 관련 있음, 클래스 변수를 사용하기 위한 함수
# 클래스 메소드는 함수를 정의할 때 첫번째 인자로 클래스를 넘겨받는 cls가 필요하며, 이를 이용해 클래스 변수에 접근함
# 클래스 레벨에서 무언가를 해야할 때 사용하기 적합
# @classmethod 데코레이터를 사용해 클래스에 메소드 선언하면 해당 메소드는 클래스 메소드가 되며,
# 첫번째 매개변수로 클래스 인스턴트가 아닌 클래스 자체가 넘어옴
# 이 첫번째 매개변수의 이름은 관행적으로 cls라고 하며
# 클래스 메소드는 이 cls를 통해 클래스 속성(attribute)에 접근하거나 클래스 메소드 호출 가능
# 인스턴스 메소드와 달리 인스턴스 속성에 접근하거나 다른 인스턴스 메소드 호출하는 것은 불가능
# 팩토리 메소드 작성시 많이 사용
...
class Car :
    ...
    # 앞에 정적메소드(@staticmethod)가 들어가서 한번만 체크하면 됨
    ...
    @classmethod
    # 클래스와 관련 있는 클래스메소드
    def print_cnt(cls) : # cls 키워드는 클래스 변수에 접근할 때 매개물로 전달받아서 사용, 클래스변수(메소드)에 접근하는 경우 사용
